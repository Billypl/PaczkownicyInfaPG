...
RE: RE RE %prec CONCAT {
  int first = !RE_NULL($1) ? RE_FIRST($1) : merge_lists(RE_FIRST($1), RE_FIRST($2));
  int last = !RE_NULL($2) ? RE_LAST($2) : merge_lists(RE_LAST($1), RE_LAST($2));
  int follow = merge_pairlist(merge_pairlist(RE_FOLLOW($1), RE_FOLLOW($2)), list_product(RE_LAST($1), RE_FIRST($2)));
  $$ = createRE(RE_NULL($1) & RE_NULL($2), first, last, follow);
};

/* alternative */
RE: RE '|' RE {
  int first = merge_lists(RE_FIRST($1), RE_FIRST($3));
  int last = merge_lists(RE_LAST($1), RE_LAST($3));
  int follow = merge_pairlist(RE_FOLLOW($1), RE_FOLLOW($3));
  $$ = createRE(RE_NULL($1) | RE_NULL($3), first, last, follow);
};
...
RE: RE '*' {
  ...
  int last = RE_LAST($1);
  int follow = merge_pairlist(RE_FOLLOW($1), list_product(RE_LAST($1), RE_FIRST($1)));
  $$ = createRE(1, first, last, follow);
};
...
RE: RE '+' {
  ...
  int last = RE_LAST($1);
  int follow = merge_pairlist(RE_FOLLOW($1), list_product(RE_LAST($1), RE_FIRST($1)));
  $$ = createRE(RE_NULL($1), first, last, follow);
};
